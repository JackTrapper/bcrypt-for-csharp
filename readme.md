...
- To hash a password:
        ...
        hash := BCrypt.HashPassword("correct battery horse staple"); //using default cost factor
    
- To hash a password specifying your own cost factor:

        hash := BCrypt.HashPassword("correct battery horse staple", 14); //specify cost factor 14
    
- To verify a password:

        Boolean passwordRehashNeeded;
        Boolean isPasswordValid = BCrypt.CheckPassword("correct battery horse stapler", expectedHash, out passwordRehashNeeded);
...
The current (1/18/2019) hard-coded default for cost is **12**. This results in 2<sup>12</sup> = 4,096 rounds during the key setup.
